...
used, and likely already present in repos throughline runs in.

Throughline independently reimplements much of that same engineering layer
...
Throughline is a thin **governance overlay** on top of the official plugins. It
...
1. **Division of labor.** Superpowers owns discovery (`brainstorming`) and the
   ...
   branch finishing). Throughline owns governance: PRD/TDD/ADR as the
...
2. **Ownership signal = the explicit command.** Invoking `/prd-author` or
   `/tdd-author` means throughline owns that phase and does NOT separately invoke
   `superpowers:brainstorming` or `writing-plans`. If superpowers artifacts
   (`docs/superpowers/{specs,plans}`) or other prior design notes exist, throughline
   **ingests** them rather than re-interviewing. When no throughline command is
...
3. **Canonical docs.** `docs/PRD.md` + `docs/tdd/` + `docs/adr/` are the governance
   ...
   throughline.
4. **Adopt, don't reinvent.** Throughline lifts proven inline patterns from
   ...
   throughline already enforces that at build time via `/implement` +
   `build-prompt.md` — importing it into the TDD would collapse the design/build
   split that is core to throughline.

## Consequences

- Throughline is non-destructive to drop into a repo already using superpowers: it
...
- **Out of scope, tracked separately:** further slimming throughline's redundant
...
- This ADR binds future throughline TDDs to the governance-only scope and the
